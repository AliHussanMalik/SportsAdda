...
export default function TeamsScreen() {
  ...
  return (
    <ScrollView style={[styles.container, { backgroundColor: theme.bg }]} contentContainerStyle={{ padding: 16 }}>
      ...
      <Text style={[styles.subtitle, { color: theme.subText }]}>Team branding, roster management, squad roles & team challenges</Text>
            ...
            >
              <TouchableOpacity style={styles.teamHeader} onPress={() => fetchTeamRoster(t.id)}>
                ...
                <View style={{ alignItems: 'flex-end' }}>
                  ...
                  <TouchableOpacity
                    style={[styles.challengeBtn, { backgroundColor: theme.accent }]}
              ...
              {roster.map((p) => (
                <View key={p.player_id} style={[styles.rosterRow, { borderBottomColor: theme.border }]}>
                  ...
                  <View style={[styles.squadRoleBadge, { backgroundColor: theme.badgeBg }]}>
                    <Text style={[styles.squadRoleText, { color: theme.accent }]}>{p.role_in_team}</Text>
                  </View>
                </View>
              ))}
            </View>
        ...
        >
          <View style={styles.modalOverlay}>
            <View style={[styles.modalContent, { backgroundColor: theme.cardBg, borderColor: theme.accent }]}>
              ...
              {/* Match Stakes Selection */}
              <Text style={[styles.inputLabel, { color: theme.text }]}>Match Stakes:</Text>
              <View style={styles.optionGroup}>
                {['Friendly Challenge', 'Turf Fee Winner Takes All', 'League Ranking Match'].map((s) => (
                  <TouchableOpacity
                    key={s}
                    style={[
                      styles.modalOptionBtn,
                      { borderColor: theme.border },
                      matchStakes === s && { backgroundColor: theme.accent, borderColor: theme.accent }
                    ]}
                    onPress={() => setMatchStakes(s)}
                  >
                    <Text style={[styles.modalOptionText, { color: matchStakes === s ? '#000' : theme.text }]}>
                      {s}
                    </Text>
                  </TouchableOpacity>
                ))}
              ...
              <View style={styles.modalActionRow}>
                ...
                <TouchableOpacity style={[styles.sendBtn, { backgroundColor: theme.accent }]} onPress={handleSendChallenge}>
                  <Text style={styles.sendBtnText}>⚔️ Send Challenge Payload</Text>
                </TouchableOpacity>
              </View>
            </View>
          </View>
...
const styles = StyleSheet.create({
  ...
  rosterTitle: { fontSize: 14, fontWeight: '800', marginBottom: 12 },
  rosterRow: {
    flexDirection: 'row',
